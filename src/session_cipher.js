...
class SessionCipher {
    ...
    async decryptWithSessions(data, sessions) {
        ...
        console.error("libsignal: Failed to decrypt message with any known session...");
        for (const e of errs) {
            console.error("libsignal: Session error:" + e, e.stack);
        }
    ...
    async decryptWhisperMessage(data) {
        ...
        return await this.queueJob(async () => {
            ...
            if (record.isClosed(result.session)) {
                ...
                console.warn("libsignal: Decrypted message with closed session.");
            }
            await this.storeRecord(record);
