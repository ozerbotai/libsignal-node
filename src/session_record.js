        ...
        if (data.registrationId) {
            ...
            for (const key in sessions) {
                if (sessions[key].indexInfo.closed === -1) {
                    console.error('libsignal: V1 session storage migration error: registrationId',
...
class SessionRecord {
    ...
    static migrate(data) {
        ...
        for (let i = 0; i < migrations.length; ++i) {
            if (run) {
                console.info("libsignal: Migrating session to:", migrations[i].version);
    ...
    closeSession(session) {
        if (this.isClosed(session)) {
            console.warn("libsignal: Session already closed", session);
            return;
        }
        // Disable log since it is too noisy
        // console.info("libsignal: Closing session:", session);
        session.indexInfo.closed = Date.now();
    }

    openSession(session) {
        if (!this.isClosed(session)) {
            console.warn("libsignal: Session already open");
        }
        console.info("libsignal: Opening session:", session);
    ...
    removeOldSessions() {
        while (Object.keys(this.sessions).length > CLOSED_SESSIONS_MAX) {
            ...
            if (oldestKey) {
                console.info("libsignal: Removing old closed session:", oldestSession);
